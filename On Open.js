...
function onOpen() {
  try {
    ...
    var isBeta = false;
    try {
      isBeta = BOM8.CONFIG && BOM8.CONFIG.VERSION && BOM8.CONFIG.VERSION.IS_BETA;
    } catch (e) {}
...
function aggiungiWatermarkBeta() {
  try {
    ...
    cella.setValue(BOM8.CONFIG.VERSION.BETA_WARNING);
    ...
    BOM8.CONFIG.LOG.info("aggiungiWatermarkBeta", "Watermark BETA aggiunto in Budget!A1");
...
function mostraGestioneRapidaOfferte() {
  try {
    ...
    if (BOM8.CONFIG && BOM8.CONFIG.LOG) {
      BOM8.CONFIG.LOG.error("mostraGestioneRapidaOfferte", "Errore", error);
    }
...
function mostraConfigurazioneOfferte() {
  try {
    ...
    if (BOM8.CONFIG && BOM8.CONFIG.LOG) {
      BOM8.CONFIG.LOG.error("mostraConfigurazioneOfferte", "Errore", error);
    }
    SpreadsheetApp.getUi().alert("Errore: " + error.toString());
  }
}
...
function isBetaVersion() {
  try {
    return BOM8.CONFIG && BOM8.CONFIG.VERSION && CONFIG.VERSION.IS_BETA;
  } catch (e) {
    return false;
  }
}
...
function azzeraOfferta(id) {
  try {
    return BOM8.azzeraOffertaSingola(id);
  } catch (error) {
 ...
 */
// function verificaFormuleTutteLeOfferte() { return BOM8.verificaFormuleTutteLeOfferte(); }
// function verificaLabelTutteLeOfferte() { return BOM8.verificaLabelTutteLeOfferte(); }
// function verificaFormuleELabelTutteLeOfferte() { return BOM8.verificaFormuleELabelTutteLeOfferte(); }
...
// function azzeraOfferta_Off_01() { return BOM8.azzeraOfferta("Off_01"); }
// function azzeraOfferta_Off_02() { return BOM8.azzeraOfferta("Off_02"); }
...
/**
 * Funzioni chiamate dal menu - delegano alla libreria BOM8
 */
function controlla() { return BOM8.controlla(); }
function rigeneraBudgetDaOfferte() { return BOM8.rigeneraBudgetDaOfferte(); }
function azzeraTutteLeOfferte() { return BOM8.azzeraTutteLeOfferte(); }
function aggiungiNuovaOfferta(nome, descrizione) { return BOM8.aggiungiNuovaOfferta(nome, descrizione); }
function rigeneraBudget() { return BOM8.rigeneraBudget(); }
