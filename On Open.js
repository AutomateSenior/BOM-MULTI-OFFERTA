...
function onOpen() {
  try {
    ...
    try {
      if (CONFIG && CONFIG.VERSION && CONFIG.VERSION.IS_BETA) {
    ...
    var isBeta = false;
    try {
      isBeta = CONFIG && CONFIG.VERSION && CONFIG.VERSION.IS_BETA;
    } catch (e) {}
  ...
  try {
    ...
    try {
      if (CONFIG && CONFIG.LOG) {
        CONFIG.LOG.warn("onOpen", "Errore creazione menu dinamico: " + e.toString());
      }
    } catch (logError) {}
  }
...
function aggiungiWatermarkBeta() {
  try {
    ...
    cella.setValue(CONFIG.VERSION.BETA_WARNING);
    ...
    CONFIG.LOG.info("aggiungiWatermarkBeta", "Watermark BETA aggiunto in Budget!A1");

  } catch (error) {
    CONFIG.LOG.error("aggiungiWatermarkBeta", "Errore", error);
  }
}
...
function azzeraOfferta(id) {
  try {
    return azzeraOffertaSingola(id);
  } catch (error) {
