  ...
  try {
    if (e && e.range) {
      ...
      if (sheetName === "Budget" && cellAddress === "L56") {
        ...
        BOMLib.CONFIG.LOG.info("Inserimento", "Modificata cella L56, avvio allineamento BOM");

        BOMLib.BOMLib.controlla(SpreadsheetApp.getActiveSpreadsheet(), false);  // Funzione locale in BOMCore.js
    ...
    BOMLib.CONFIG.LOG.error("Inserimento", "Errore trigger L56", error);
  }

  // Gestione trigger standard BOM (solo se non è L56)
  try {
    BOMLib.BOMLib.InserimentoBOM(e);  // Funzione locale in BOMCore.js
  } catch (error) {
    BOMLib.CONFIG.LOG.error("Inserimento", "Errore InserimentoBOM", error);
  }
}
